...
public interface ThrowingPrintable<X extends Throwable> extends Printable
{
    @Override
    @SneakyThrows
    default int print(java.awt.Graphics graphics, java.awt.print.PageFormat pageFormat, int pageIndex)
    {
        return tryPrint(graphics, pageFormat, pageIndex);
    }

    /**
     * Prints the page at the specified index into the specified {@link Graphics}
     * context in the specified format. A <code>PrinterJob</code> calls the
     * <code>Printable</code> interface to request that a page be rendered into the
     * context specified by <code>graphics</code>. The format of the page to be
     * drawn is specified by <code>pageFormat</code>. The zero based index of the
     * requested page is specified by <code>pageIndex</code>. If the requested page
     * does not exist then this method returns NO_SUCH_PAGE; otherwise PAGE_EXISTS
     * is returned. The <code>Graphics</code> class or subclass implements the
     * {@link PrinterGraphics} interface to provide additional information. If the
     * <code>Printable</code> object aborts the print job then it throws a
     ...
     */
    int tryPrint(java.awt.Graphics graphics, java.awt.print.PageFormat pageFormat, int pageIndex) throws X;
}
