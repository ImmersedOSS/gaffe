...
public interface ThrowingReadable<X extends Throwable> extends Readable
{
    @Override
    @SneakyThrows
    default int read(java.nio.CharBuffer cb)
    {
        return tryRead(cb);
    }
     ...
     * @return The number of {@code char} values added to the buffer, or -1 if this
     ...
     */
    int tryRead(java.nio.CharBuffer cb) throws X;
}
