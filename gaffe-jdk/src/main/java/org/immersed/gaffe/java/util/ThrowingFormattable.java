...
public interface ThrowingFormattable<X extends Throwable> extends Formattable
{
    ...
    {
        tryFormatTo(formatter, flags, width, precision);
    }

    /**
     * Formats the object using the provided {@link Formatter formatter}.
     *
     * @param formatter The {@link Formatter formatter}. Implementing classes may
     *                  call {@link Formatter#out() formatter.out()} or
     ...
     *
     * @param flags     The flags modify the output format. The value is interpreted
     ...
     *                  the <tt>width</tt> then the output will be padded by
     *                  <tt>'&nbsp;&nbsp;'</tt> until the total number of characters
     *                  equals width. The padding is at the beginning by default. If
     *                  the {@link FormattableFlags#LEFT_JUSTIFY} flag is set then
     *                  the padding will be at the end. If <tt>width</tt> is
     *                  <tt>-1</tt> then there is no minimum.
     *
     * @param precision The maximum number of characters to be written to the
     *                  output. The precision is applied before the width, thus the
     *                  output will be truncated to <tt>precision</tt> characters
     *                  even if the <tt>width</tt> is greater than the
     *                  <tt>precision</tt>. If <tt>precision</tt> is <tt>-1</tt>
     *                  then there is no explicit limit on the number of characters.
     *
     * @throws IllegalFormatException If any of the parameters are invalid. For
     *                                specification of all possible formatting
     *                                errors, see the <a href=
     *                                "../util/Formatter.html#detail">Details</a>
     *                                section of the formatter class specification.
     *
     * @throws X                      any exception that may be thrown.
     */
    void tryFormatTo(java.util.Formatter formatter, int flags, int width, int precision) throws X;
}
