 ...
 * {@link MediaPlayer} using {@link MediaPlayer#setAudioSpectrumListener}.
 *
 * @see MediaPlayer
 * @since JavaFX 2.0
...
public interface ThrowingAudioSpectrumListener<X extends Throwable> extends AudioSpectrumListener
{
    @Override
    @SneakyThrows
    default void spectrumDataUpdate(double timestamp, double duration, float[] magnitudes, float[] phases)
    {
     ...
     *
     * <p>
     * The arrays passed to the listener should be considered to be read-only as the
     * underlying implementation may re-use them.
     * </p>
     *
     * @param timestamp  timestamp of the event in seconds.
     * @param duration   duration for which the spectrum was computed in seconds.
     * @param magnitudes array containing the non-positive spectrum magnitude in
     *                   decibels (dB) for each band. The size of the array equals
     *                   the number of bands and should be considered to be
     *                   read-only.
     * @param phases     array containing the phase in the range
     *                   [<code>Math.PI</code>,&nbsp;<code>Math.PI</code>] for each
     *                   band. The size of the array equals the number of bands and
     *                   should be considered to be read-only.
     *
     * @throws X any exception that may be thrown.
     */
    void trySpectrumDataUpdate(double timestamp, double duration, float[] magnitudes, float[] phases) throws X;
}
